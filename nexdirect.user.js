...
(function() {
	...
	function injectDownloadPage() {
		...
		var $posttextOnclick = $("div.posttext a").attr("onclick"); // jack from the beatmappreview script inline
		if (!$posttextOnclick)
			return log("Could not find beatmap preview element."); // rip

        log("Found beatmap preview element.");
		
		var beatmapSetId = $posttextOnclick.match(/playBeatmapPreview\((\d+)\)/);
		...
		if (!$downloadButton || $downloadButton.length < 1)
			return log("Could not find the beatmap download button.");
